    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="information" label="详细信息" align="center">
          <!-- <router-link to="/disaterinform">灾情详情</router-link> -->
          <template #default="scope">
            <el-button
              type="primary"
              :icon="Plus"
              @click="handleRouteToDisaterinform(scope.$index)"
              >灾情详情</el-button
            ></template
          >
        </el-table-column>
...
const query = reactive({
  //用于查询的各种属性，如分页信息、灾害 ID、位置、时间、资源信息
  pageIndex: 1,
  pageSize: 10,
  disaterId: "", //灾情一体化信息
  location: "",
...
// 跳转到灾情详情页面
const handleRouteToDisaterinform = (index: number) => {
  router.push({
    path: "/disaterinform",
    query: {
      code: tableData.value[index].disasterCode,
      carrier: tableData.value[index].carrierType,
      code_source: tableData.value[index].sourceInfo,
      date: tableData.value[index].date,
      code_details: tableData.value[index].disasterInfo,
      address: tableData.value[index].location,
    },
  });
  console.log("testHttpPost 响应", tableData.value[index]);
};
// 查询操作
const handleSearch = async () => {
  ...
  const datePart = query.time
    ? new Date(query.time).toLocaleDateString("en-CA")
    : "";
